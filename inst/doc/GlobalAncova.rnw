...
\author{Manuela Hummel \and Reinhard Meister \and Ulrich Mansmann} %\and Ramona Scheufele \and Sven Knueppel}
...
The result of the \Rfunction{GlobalAncova} function is a typical ANOVA table with information about sums of squares, degrees of freedom and mean sums of squares for the effect and error term, respectively. 
...
%Which of them are calculated is controled by the argument \Rfunarg{method}.  
...
This creates a \Rclass{GOstructure} object (class from \Rpackage{globaltest}) for the Golub data, using
