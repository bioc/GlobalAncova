...
data(Golub_Merge)
golubX <- vsn(exprs(Golub_Merge))
pData(golubX) <- pData(Golub_Merge)
@

This creates a dataset \Robject{golubX}, which is of the format
\Rclass{ExpressionSet}, the standard format for gene expression data in
...
Those names can be chosen by previous output of the \Rfunction{GlobalAncova} 
