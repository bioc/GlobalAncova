...
and the technical report of Hummel, Meister and Mansmann \cite{Hummel:07}.
...
asymptotic p--values. 
%To use the approximation specify the parameter \Rfunarg{method = "approx"} (\Rfunarg{method = "permutation"} is the default) or \Rfunarg{method = "both"} to get both p--values. 
...
We thank Sven Knüppel who took the initiative and contributed a C-code
