...
are available from the Bioconductor website ({\it www.bioconductor.org}) and 
from CRAN ({\it www.r-project.org}), respectively.
...
We choose only the four pathways that have p--values $< 0.05$ for demonstrating the 
adjustment because the other one is not significant anyway.
...
<<>>=
ga.kegg.raw <- ga.kegg[c(1,2,3,5), 3]
ga.kegg.adj <- mt.rawp2adjp(ga.kegg.raw)
ga.kegg.adj$adjp[order(ga.kegg.adj$index) ,c("rawp", "Holm")]
@

The adjusted p--values are in general larger than the raw p--values
but still all four pathways are significant.
...
of all four selected pathways are not differentially expressed.
Now the interesting hypothesis ``1'' for example can be rejected if also the hypotheses
``1-2-3-4'',``1-2-3'',``1-2-4'',``1-3-4'',``1-2'',``1-3'' and ``1-4'' are rejected.
...
{
  res <- Hnull.family(kegg[c(1,2,3,5)])
...
<<closed.test>>=
ga.closed <- GlobalAncova.closed(xx=exprs(golubX), group=gr, test.genes=kegg[c(1,2,3,5)], previous.test=ga.kegg, level=0.05, perm=1000)
@
...
The function \Rfunction{GlobalAncova.closed} provides the formed null hypotheses (this means lists of genes to be tested simultaneously), the test results for each pathway of interest and the names of significant and not significant pathways.
...
Here also all four pathways turn out to be significant.
